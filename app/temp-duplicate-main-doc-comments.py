...
from app.api.media import (
    bulk_router as assets_bulk_router,
    global_media_router,
    media_router,
    registry_router,
...
LOGGER = logging.getLogger("media_sync_api.main")
...
# ------------------------------------------------------------------------------
# 🧭 OPERATOR ROUTE MAP (LAN / ROUTER / DEVICE FLOW)
# ------------------------------------------------------------------------------
"""
...
Explorer (dev frontend):
    http://192.168.0.25:3000

Recommended HTTPS (via Caddy):
    https://media-sync.local

Core Surfaces
-------------
/connect
    Discovery + onboarding surface (browser + device entrypoint)

/connect/register
    Register node (browser session, Pi, daemon, etc)

/connect/device
    Device activation page (camera, screen, recording)

/api/nodes
    Control-plane registry of all known runtimes

/api/sources
    Merged canonical + remote source inventory

/api/live_sessions
    Live session control + transport (camera, screen, WebRTC)

/api/ingest/claims
    Canonical ingest acceptance pipeline

/media/*
    Stream + download finalized assets

/player.html
    Playback surface for media URLs

/routes
    Runtime route introspection map

/health
    Operator health + runtime service readiness

Typical LAN Flow (iPhone example)
---------------------------------
1. Open Explorer:
    http://192.168.0.25:3000
...
5. Preview + record:
    → chunks → /api/live_sessions/{id}/chunk

6. End session:
    → POST /api/live_sessions/{id}/end

7. Asset appears in Explorer

Router / DNS Notes
------------------
If using local DNS (Pi-hole / router):
    media-sync.local → 192.168.0.25

If using HTTPS proxy (Caddy):
    https://media-sync.local → this service
...
Node Types
----------
Daemon Node (Pi, server):
    base_url = http://<ip>:<port>

Browser / iPhone:
    base_url = null
    transport = session (push-based)

Authority owns all endpoints.
Devices push data; they are not servers.
"""
# ------------------------------------------------------------------------------
...
async def lifespan(app: FastAPI):
    ...
    LOGGER.info("media-sync-api started at http://0.0.0.0:%s", settings.port)
    LOGGER.info("Explorer dev expected at http://192.168.0.25:3000")
    LOGGER.info("Connect surface: /connect")
    LOGGER.info("Route map surface: /routes")
...
def create_app() -> FastAPI:
    ...
    async def public_index():
        ...
        return {
            ...
            "instructions": (
                "Place public/index.html next to the app package "
    ...
    async def public_player():
        ...
        return {
            ...
            "instructions": (
                "Place public/player.html next to the app package "
                "or rebuild the image."
            ),
        }

    @application.get("/routes", include_in_schema=False)
    ...
    async def healthcheck():
        ...
        return {
            ...
            "ingest_claims_enabled": (
                ingest_registry_ready and ingest_claim_service_ready
            ),
            ...
            "remote_source_records": len(
                runtime.metadata.get("remote_source_records", [])
            ),
            "operator": {
                "explorer_dev": "http://192.168.0.25:3000",
                "api": "http://192.168.0.25:8787",
                "connect": "http://192.168.0.25:8787/connect",
                "device": "http://192.168.0.25:8787/connect/device",
                "routes": "http://192.168.0.25:8787/routes",
                "player": "http://192.168.0.25:8787/player.html",
                "recommended_https": "https://media-sync.local",
            },
            "instructions": (
                "See /connect for authority discovery, /routes for live route "
                "introspection, and /public/index.html for end-to-end adapter guidance."
            ),
        }
