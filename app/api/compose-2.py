...
def _resolve_within(ctx: ProjectContext, relative: str, *, require_exists: bool) -> Path:
...
def load(cls, settings: Any, project_name: str, run_id: str) -> "ComposeSession | None":
    ...
    return cls(
        ...
        created_ts=meta.get("created_ts", datetime.now(timezone.utc).timestamp()),
    )
...
Future: segment selection, duration targeting, album rules.
"""
...
def build_existing_plan(self, ctx: ProjectContext, spec: ComposeSpec) -> ComposePlan:
    ...
    input_paths = [_resolve_within(ctx, rel, require_exists=True) for rel in normalized]
...
def _resolve_output_path(self, ctx: ProjectContext, spec: ComposeSpec) -> Path:
    target_dir = _resolve_within(ctx, spec.target_dir.strip() or "exports", require_exists=False)
...
def execute(self, plan: ComposePlan) -> ComposeResult:
    if plan.strategy == "copy":
        return self._run_copy(plan)
    return self._run_encode(plan)
...
def _run_copy(self, plan: ComposePlan) -> ComposeResult:
    ...
    if result.returncode != 0:
        ...
        # auto mode: fall through to encode
        return self._run_encode(plan)
    return ComposeResult(output_path=plan.output_path, mode_used="copy")

def _run_encode(self, plan: ComposePlan) -> ComposeResult:
    list_path = self._write_concat_list(plan)
    command = [
        "ffmpeg", "-n",
        "-f", "concat", "-safe", "0",
...
def register(
    ...
    if existing and existing != output_rel:
        return self._handle_duplicate(ctx, active_source, existing, sha256, result.mode_used, base_url)

    return self._handle_new(ctx, active_source, output_rel, sha256, result, base_url)
...
def _handle_duplicate(
    ...
    # Remove duplicate file (already exists elsewhere)
    removed_shas = remove_file_entries_for_relative_path(project, existing_rel)
    remove_file_hashes_by_relative_path(manifest_db, existing_rel)
    ...
    return {
        ...
        "sha256": sha256,
        "mode_used": mode_used,
        "served": self._served_urls(base_url, ctx.project_name, existing_rel, active_source.name),
    }
...
def _handle_new(
    ...
    ensure_metadata(project, output_rel, sha256, result.output_path, source=active_source.name, method="compose")
    ...
    return {
        ...
        "served": self._served_urls(base_url, ctx.project_name, output_rel, active_source.name),
    }
...
def compose_existing(
    ...
    return self.registrar.register(ctx, result, request, active_source)
...
async def compose_upload_batch(
    ...
    try:
        ...
        result = self.executor.execute(plan)
        response = self.registrar.register(ctx, result, request, active_source)
        logger.info(
...
ctx, active_source = _resolve_project_context(project_name, source)
...
return ComposeService().compose_existing(ctx, spec, request, active_source)
...
if is_incremental:
    return await service.compose_upload_incremental(
        ctx, spec, files[0], request, active_source, settings,
        run_id=run_id, idx=idx, total=total,
    )

return await service.compose_upload_batch(ctx, spec, files, request, active_source, settings)
