...
class ComposeRequest(BaseModel):
    ...
    debug_keep_intermediates: bool = Field(
        default=False,
        description="Preserve the per-job normalized/intermediate work dir for operator debugging.",
    )
...
class ComposeJob:
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            ...
            "debug_keep_intermediates": self.debug_keep_intermediates,
...
def _compose_debug_root() -> Path:
    root = Path(get_settings().temp_root) / "compose_debug"
    root.mkdir(parents=True, exist_ok=True)
    return root


def _preserve_compose_debug_artifacts(work_dir: Path, *, job_id: str | None = None) -> dict[str, Any] | None:
    if not work_dir.exists():
        return None
    target_root = _compose_debug_root() / (job_id or work_dir.name)
    shutil.rmtree(target_root, ignore_errors=True)
    try:
        shutil.copytree(work_dir, target_root)
    except Exception as exc:
        _log_compose(
            "warning",
            "compose_debug_artifacts_preserve_failed",
            job_id=job_id,
    ...
    payload = {"work_dir": str(target_root), "files": files}
    normalized_dir = target_root / "normalized"
    if normalized_dir.exists():
        payload["normalized_dir"] = str(normalized_dir)
    _log_compose(
        "info",
        "compose_debug_artifacts_preserved",
        job_id=job_id,
        debug_root=str(target_root),
...
def _serialize_compose_job(job: ComposeJob, *, base_url: str | None = None) -> dict[str, Any]:
    payload = {
        ...
        "debug_keep_intermediates": job.debug_keep_intermediates,
...
class ComposeRegistrar:
    ...
    @staticmethod
    def _attach_debug_artifacts(payload: dict[str, Any], result: ComposeResult) -> dict[str, Any]:
        if result.debug_artifacts is not None:
    ...
    def _handle_preserve_run(
        ...
        return self._attach_debug_artifacts({
            "status": "stored",
...
class ComposeService:
    ...
    def _run_encode_compose(
        ...
        *,
        ...
        keep_debug_artifacts = bool(getattr(spec, "debug_keep_intermediates", False))
        try:
            ...
            result = self.executor.execute(encode_plan, job_id=job_id)
        except HTTPException as exc:
            if keep_debug_artifacts:
                debug_artifacts = _preserve_compose_debug_artifacts(work_dir, job_id=job_id)
                if debug_artifacts is not None:
                    raise HTTPException(
                        status_code=exc.status_code,
                        detail=f"{exc.detail} [debug_artifacts={debug_artifacts}]",
                    ) from exc
            raise
        if keep_debug_artifacts:
            debug_artifacts = _preserve_compose_debug_artifacts(work_dir, job_id=job_id)
            if debug_artifacts is not None:
                result = replace(result, debug_artifacts=debug_artifacts)
        return result

    def _run_auto_compose(
        self,
        plan: ComposePlan,
        spec: ComposeSpec,
        work_dir: Path,
        *,
        ...
        return self._run_encode_compose(plan, spec, work_dir, job_id=job_id)

    def _execute_plan(self, plan: ComposePlan, spec: ComposeSpec, work_dir: Path, *, job_id: str | None = None) -> ComposeResult:
        ...
        if plan.requested_mode == "copy":
            ...
            result = self._run_encode_compose(plan, spec, work_dir, job_id=job_id)
        else:
            result = self._run_auto_compose(plan, spec, work_dir, job_id=job_id)
    ...
    def compose_staged_paths(
        ...
        *,
        ...
        result = self._execute_plan(plan, spec, work_dir, job_id=job_id)
        logger.info(
...
async def compose_upload(
    ...
    debug_keep_intermediates: bool = Query(default=False),
):
