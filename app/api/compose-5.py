...
class ComposeExecutor:
    ...
    def _write_concat_list(self, plan: ComposePlan) -> Path:
        ...
        # Use prepared_segments when available (future: preprocessed image/audio segments).
        # Fall back to raw input_paths for backward compatibility.
        sources = plan.prepared_segments or [
            PreparedSegment(path=p, source_kind="video", generated=False)
            for p in plan.input_paths
        ]
