...
def load(cls, settings: Any, project_name: str, run_id: str) -> "ComposeSession | None":
    ...
    return cls(
        ...
        count=int(meta["count"]) if isinstance(meta.get("count"), (int, float)) and meta["count"] > 0 else 0,
        received=sorted({
...
def register(
    ...
    if existing and existing != output_rel:
        return self._handle_duplicate(ctx, active_source, existing, sha256, result.output_path, result.mode_used, base_url)
