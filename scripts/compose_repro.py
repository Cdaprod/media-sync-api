...
def collect_docker_compose_logs(*, compose_file: str, service: str, since: str | None = None) -> list[str]:
    """Collect docker compose logs for a service since a given UTC timestamp."""
    command = ["docker", "compose", "-f", compose_file, "logs", service]
    if since:
...
def collect_compose_job_log_block(*, compose_file: str, service: str, since: str, job_id: str) -> tuple[list[str], str]:
    """Collect job lifecycle logs, retrying with a full service scan if the recent window is empty."""
    for scope, attempt_since in (("recent", since), ("full", None)):
        lines = collect_docker_compose_logs(compose_file=compose_file, service=service, since=attempt_since)
...
def _build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--debug-keep-intermediates",
        action="store_true",
        help="Ask the backend to preserve the per-job work dir (including normalized intermediates) for debugging.",
    )
    parser.add_argument(
        "--relative-path",
        action="append",
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    try:
        payload = build_existing_compose_payload(
            ...
            debug_keep_intermediates=args.debug_keep_intermediates,
        )
        ...
        if args.docker_service:
            selected, scope = collect_compose_job_log_block(
