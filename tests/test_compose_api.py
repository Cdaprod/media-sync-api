...
def test_compose_existing_accepts_job_and_registers_one_asset(client, monkeypatch):
    ...
    assert body["debug_keep_intermediates"] is False
...
def test_compose_service_preserves_debug_artifacts_when_requested(monkeypatch, tmp_path: Path):
    first = tmp_path / "a.mov"
    second = tmp_path / "b.mov"
    output = tmp_path / "compiled-0001.mp4"
    first.write_bytes(b"a")
    ...
    spec = type("Spec", (), {"mode": "encode", "debug_keep_intermediates": True})()
    plan = ComposePlan(
        input_paths=[first, second],
        output_path=output,
        strategy="encode",
    ...
    def _fake_prepare(assets, work_dir, job_id=None):
        normalized_dir = work_dir / "normalized"
        normalized_dir.mkdir(parents=True, exist_ok=True)
        segment = normalized_dir / "segment_0000.mp4"
        segment.write_bytes(b"normalized")
        return [PreparedSegment(path=segment, source_kind="video", generated=True)]

    monkeypatch.setattr(service.preprocessor, "prepare", _fake_prepare)
    monkeypatch.setattr(service.executor, "execute", lambda plan, job_id=None: ComposeResult(output_path=output, mode_used=plan.strategy))
    monkeypatch.setattr(
        service.registrar,
        "register",
        lambda _ctx, result, _base_url: {"path": result.output_path.name, "debug_artifacts": result.debug_artifacts},
    )
    monkeypatch.setattr(
        "app.api.compose._probe_media_summary",
        lambda path: {
    ...
    result = service.compose_staged_paths(ctx, spec, [first, second], "http://localhost:8787", work_dir=tmp_path / "work", job_id="job-debug")

    debug_artifacts = result["debug_artifacts"]
    assert debug_artifacts["normalized_dir"].endswith("compose_debug/job-debug/normalized")
    assert "normalized/segment_0000.mp4" in debug_artifacts["files"]
