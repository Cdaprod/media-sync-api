...
const CHANNEL_NAME = 'media-sync-browser-runtime';
...
export const publishBrowserRuntimeIdentity = (): void => { const channel = openChannel(); if (!channel) return; const identity = getBrowserRuntimeIdentity(); channel.postMessage({ type: 'identity', nodeId: identity.nodeId, hasToken: Boolean(identity.token), tokenSource: identity.tokenSource }); channel.close(); };
export const publishBrowserRuntimeSession = (sessionId: string | null, nodeId?: string | null): void => { const channel = openChannel(); if (!channel) return; channel.postMessage({ type: 'session', nodeId: nodeId || getStoredNodeId(), sessionId, role: getTabRole() }); channel.close(); };
