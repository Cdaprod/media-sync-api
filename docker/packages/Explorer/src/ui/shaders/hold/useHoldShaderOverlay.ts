...
uniform float u_confirm;
void main(){
  vec2 p = v_uv - u_center;
  p.x *= u_res.x / max(u_res.y, 1.0);
  float d = length(p);
  float pi = 3.14159265;
  float ringR = 0.052 - u_confirm * 0.004;
  float ringW = 0.0045;
  float ring = smoothstep(ringR + ringW, ringR, d) * (1.0 - smoothstep(ringR, ringR - ringW, d));

  float haloR = ringR + 0.024;
  float halo = smoothstep(haloR, ringR - 0.004, d) * (0.16 + u_confirm * 0.18);

  float angle = atan(p.y, p.x);
  float clockwise = fract(1.25 - angle / (2.0 * pi));
  float head = clamp(u_progress, 0.0, 1.0);
  float tail = max(0.0, head - 0.16);
  float arcMask = smoothstep(tail, tail + 0.015, clockwise) * (1.0 - smoothstep(head, head + 0.015, clockwise));
  float arc = ring * arcMask;
  float progressGlow = ring * smoothstep(0.0, 1.0, u_progress) * 0.42;

  vec3 baseCol = vec3(0.60, 0.36, 0.96);
  vec3 arcCol = vec3(0.86, 0.70, 1.0);
  float completionSweep = ring * u_confirm * 0.78;
  float completionPop = u_complete * (0.55 + halo * 0.55);

  vec3 col = baseCol * (ring * (0.35 + u_active * 0.35) + halo * 0.45);
  col += arcCol * (arc * 0.88 + progressGlow * 0.16);
  col += arcCol * (completionSweep * 0.8 + completionPop);
  float visibility = max(u_active * 0.85, u_confirm);
  float alpha = (ring * 0.52 + halo * 0.32 + arc * 0.58 + completionSweep * 0.68 + completionPop * 0.7) * visibility;
...
export function useHoldShaderOverlay() {
  ...
  const completionVisibilityRef = useRef(0);
  ...
  useEffect(() => {
    ...
    const uConfirm = gl.getUniformLocation(program, 'u_confirm');
    ...
    const render = (now: number) => {
      ...
      prev = now;
      completionRef.current = Math.max(0, completionRef.current - dt * 3.8);
      completionVisibilityRef.current = Math.max(0, completionVisibilityRef.current - dt * 2.1);
      ...
      if (uConfirm) gl.uniform1f(uConfirm, completionVisibilityRef.current);
  ...
  const setHoldState = useCallback((point: OverlayPoint, active: boolean, progress: number, completionBeat: number) => {
    ...
    if (completionBeat !== completionBeatRef.current) {
      ...
      completionVisibilityRef.current = 1;
    }
  }, []);
