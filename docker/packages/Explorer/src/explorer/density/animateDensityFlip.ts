...
const overlayFadeTimerByGrid = new WeakMap<HTMLElement, number>();
...
export function animateDensityFlip({
  ...
  const setDensityMotionActive = (active: boolean) => {
    ...
    if (active) {
      clearSettlingTimer();
      const pendingOverlayTimer = overlayFadeTimerByGrid.get(gridEl);
      if (pendingOverlayTimer != null) {
        window.clearTimeout(pendingOverlayTimer);
        overlayFadeTimerByGrid.delete(gridEl);
      }
      contentEl.classList.remove('density-overlay-in');
      contentEl.classList.add('density-overlay-out');
    ...
    contentEl.classList.remove('density-overlay-out');
    if (!contentEl.classList.contains('overlay-hidden')) {
      contentEl.classList.add('density-overlay-in');
      const overlayInTimer = window.setTimeout(() => {
        contentEl.classList.remove('density-overlay-in');
        overlayFadeTimerByGrid.delete(gridEl);
      }, 210);
      overlayFadeTimerByGrid.set(gridEl, overlayInTimer);
    }
