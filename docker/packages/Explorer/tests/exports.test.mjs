...
test('device live session start uses POST device lane and never GETs start as a session id', () => {
  ...
  assert.ok(startBody.includes("fetch(buildUrl('/api/live_sessions/start')"));
  assert.ok(startBody.includes('const payload = {'));
  assert.ok(startBody.includes('durableLiveSessionCreatePayload: payload'));
  assert.ok(startBody.includes('durableLiveSessionCreateStatus: null as number | null'));
  assert.ok(startBody.includes('durableLiveSessionCreateResponse: null as unknown'));
  assert.ok(startBody.includes('markConnectDeviceBroadcastDebug(liveSessionCreateDiagnostics);'));
  assert.ok(startBody.includes('liveSessionCreateAttempted: true'));
  assert.ok(startBody.includes('liveSessionCreateSucceeded: false'));
  assert.ok(startBody.includes('liveSessionCreateError: null as string | null'));
  assert.ok(startBody.includes('liveSessionCreateDiagnostics.liveSessionCreateSucceeded = Boolean(record?.session_id);'));
  ...
  assert.ok(!/liveSessionCreateAttempted: true,[\s\S]*liveSessionCreateAttempted: true,[\s\S]*liveSessionCreateSucceeded: false/.test(startBody));
  assert.ok(startBody.includes('const authHeaders = getNodeAuthHeaders(nodeId);'));
  assert.ok(!startBody.includes('authHeaders = requireNodeAuthHeaders(nodeId);'));
