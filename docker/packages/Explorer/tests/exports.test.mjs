...
test('package explorer topbar layout follows static two-row structure', () => {
  ...
  assert.ok(content.includes('const retainedPrefsHydratedRef = useRef(false);'));
  assert.ok(content.includes('window.localStorage.getItem(RETAINED_UI_PREFS_KEY)'));
  assert.ok(content.includes('retainedPrefsHydratedRef.current = false;'));
  assert.ok(content.includes('retainedPrefsHydratedRef.current = true;'));
  assert.ok(content.includes('if (!retainedPrefsHydratedRef.current) {'));
