...
test('package explorer topbar layout follows static two-row structure', () => {
  ...
  assert.ok(content.includes('if (VALID_SORT_KEYS.has(parsed.sortKey as SortKey)) {'));
  assert.ok(content.includes('if (VALID_TYPE_FILTERS.has(parsed.typeFilter as MediaTypeFilter)) {'));
  assert.ok(content.includes('if (typeof parsed.selectedOnly === \'boolean\') {'));
  assert.ok(content.includes('if (typeof parsed.untaggedOnly === \'boolean\') {'));
  assert.ok(content.includes('if (typeof parsed.overlayEnabled === \'boolean\') {'));
  ...
  assert.ok(content.includes('window.localStorage.getItem(LEGACY_FILTER_PREFS_KEY)'));
